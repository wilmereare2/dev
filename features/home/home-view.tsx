...
/** Shared horizontal rhythm — shell provides outer padding on full-width pages. */
...
export function HomeView({ data, defaults }: HomeViewProps) {
  ...
  return (
    <div className="pb-10">
      <section className="relative overflow-hidden border-b border-border/60">
        ...
        <div
          className={cn(
            "relative w-full",
            hasContent
              ? "grid gap-6 py-10 sm:gap-8 sm:py-12 lg:grid-cols-2 lg:items-center lg:py-16"
              : "py-10 sm:py-12 lg:py-14",
          )}
        >
          <div className={hasContent ? "min-w-0" : "mx-auto max-w-2xl text-center lg:mx-0 lg:text-left"}>
            <motion.p
            ...
            <motion.div
              ...
              className={cn("mt-6 flex flex-wrap gap-3", !hasContent && "justify-center lg:justify-start")}
            >
          ...
          {featured ? (
            <motion.div
              ...
              className="min-w-0"
            >
              <ContentCard item={featured} size="large" priority className="h-full" />
            </motion.div>
          ) : null}
        </div>
      </section>
      ...
      <section className={cn(sectionClass, "pt-6")}>
        <AdSlot placement="homepage_top" />
      </section>

      <section className={cn(sectionClass, "py-6")}>
        <SectionHeader title="Member promotions" href="/promotions" />
        <Link
          href="/promotions"
          className="mt-4 flex min-h-[11rem] flex-col justify-center rounded-2xl border border-accent/25 bg-gradient-to-r from-accent/10 via-surface/50 to-surface/50 p-5 transition hover:border-accent/45 sm:min-h-[12rem] sm:p-6"
        >
      ...
      {!hasContent ? (
        <section className={cn(sectionClass, "py-10 sm:py-12")}>
          <HomeLaunchSections />
        </section>
      ) : null}

      <section className={cn(sectionClass, "py-8")}>
      ...
      <section className={cn(sectionClass, "py-8")}>
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_min(100%,300px)]">
          <div className="min-w-0">
            <SectionHeader
              title={hasContent ? "Latest releases" : "Upcoming releases"}
              href={hasContent ? "/newest" : undefined}
            />
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-3 xl:grid-cols-4">
              {hasContent ? (
                ...
                <>
                  ...
                  <div className="col-span-2 flex items-center justify-center rounded-2xl border border-dashed border-border/80 bg-surface/80 p-6 sm:col-span-3 xl:col-span-4">
      ...
      {data.trending.length > 1 ? (
        <section className={cn(sectionClass, "pb-10")}>
